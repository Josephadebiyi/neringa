...
const _africanCurrencies = {
  'NGN', 'GHS', 'KES', 'ZAR', 'UGX', 'TZS', 'RWF', 'EGP', 'MAD', 'XOF', 'XAF', 'ETB',
};
...
class WithdrawalSetupScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<WithdrawalSetupScreen> createState() => _WithdrawalSetupScreenState();
}
          ...
          child: Center(
            child: Padding(
              ...
              child: Text(
                ...
                style: AppTextStyles.bodyLg.copyWith(color: AppColors.gray400, height: 1.5),
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: const Icon(Icons.account_balance_wallet_outlined, color: AppColors.primary, size: 32),
              ),
              const SizedBox(height: 24),
              Text(
                'Set up\nwithdrawals',
                style: AppTextStyles.h1.copyWith(color: AppColors.white, fontWeight: FontWeight.w800),
              ),
              ...
              TextButton(
                ...
                child: Text(
                  'Skip for now',
                  style: AppTextStyles.bodyMd.copyWith(color: AppColors.gray500, decoration: TextDecoration.underline),
                ),
              ),
              const SizedBox(height: 24),
            ],
...
class _PaystackSetupState extends ConsumerState<_PaystackSetup> {
  ...
  Future<void> _loadBanks() async {
    ...
    try {
      final response = await ApiService.instance.get(ApiConstants.paystackBanks);
      ...
      setState(() {
        _banks = bankList.map((b) => Map<String, dynamic>.from(b as Map)).toList();
        _isLoadingBanks = false;
      });
    } catch (e) {
      if (mounted) {
        setState(() => _isLoadingBanks = false);
        AppSnackBar.show(context, message: 'Could not load banks. Please try again.', type: SnackBarType.error);
      }
    }
  }
  ...
  Future<void> _resolveAccount() async {
    ...
    setState(() { _isResolving = true; _accountName = null; });
    try {
      final bankCode = _selectedBank!['code']?.toString() ?? _selectedBank!['bank_code']?.toString() ?? '';
      ...
      final name = resolved is Map
          ? (resolved['data']?['account_name'] ?? resolved['account_name'] ?? resolved['accountName'])?.toString()
          : null;
      if (mounted) {
        setState(() {
      ...
      if (mounted) {
        ...
        AppSnackBar.show(context, message: msg.toString(), type: SnackBarType.error);
      }
    } catch (_) {
      if (mounted) {
        setState(() => _isResolving = false);
  ...
  Future<void> _submit() async {
    if (_selectedBank == null) {
      AppSnackBar.show(context, message: 'Please select a bank', type: SnackBarType.error);
      return;
    }
    if (_accountCtrl.text.trim().length != 10) {
      AppSnackBar.show(context, message: 'Please enter a valid 10-digit account number', type: SnackBarType.error);
      return;
    }
    if (_accountName == null) {
      AppSnackBar.show(context, message: 'Please wait for account verification', type: SnackBarType.error);
      return;
    }
    setState(() => _isSubmitting = true);
    try {
      final bankCode = _selectedBank!['code']?.toString() ?? _selectedBank!['bank_code']?.toString() ?? '';
      await ApiService.instance.post(ApiConstants.paystackAddBank, data: {
        'account_number': _accountCtrl.text.trim(),
        'bank_code': bankCode,
      ...
      if (mounted) {
        AppSnackBar.show(context, message: 'Bank account added successfully!', type: SnackBarType.success);
      ...
      if (mounted) {
        AppSnackBar.show(context, message: 'Something went wrong. Please try again.', type: SnackBarType.error);
        setState(() => _isSubmitting = false);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _infoChip('Bank Transfer (${widget.currency})', Icons.account_balance_outlined, AppColors.primary),
        const SizedBox(height: 24),
        // Bank selector
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: _selectedBank != null ? AppColors.primary : AppColors.gray700),
            ),
            child: Row(
              children: [
                const Icon(Icons.account_balance_outlined, color: AppColors.gray400, size: 20),
                ...
                Expanded(
                  child: _isLoadingBanks
                      ...
                      : Text(
                          _selectedBank != null
                              ? (_selectedBank!['name']?.toString() ?? 'Bank selected')
                              : 'Select your bank',
                          style: AppTextStyles.bodyMd.copyWith(
                            color: _selectedBank != null ? AppColors.white : AppColors.gray500,
                          ),
                        ),
                ),
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    _accountName!,
                    style: AppTextStyles.labelMd.copyWith(color: AppColors.success, fontWeight: FontWeight.w700),
                  ),
                ),
              ],
            ),
  ...
  void initState() {
    ...
    _searchCtrl.addListener(() {
      ...
      setState(() {
        _filtered = q.isEmpty
            ? widget.banks
            : widget.banks.where((b) => (b['name']?.toString() ?? '').toLowerCase().contains(q)).toList();
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    return DraggableScrollableSheet(
      ...
      builder: (ctx, sc) => Column(
        children: [
          const SizedBox(height: 12),
          Container(width: 40, height: 4, decoration: BoxDecoration(color: AppColors.gray700, borderRadius: BorderRadius.circular(2))),
          const SizedBox(height: 16),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Text('Select Bank', style: AppTextStyles.h4.copyWith(color: AppColors.white, fontWeight: FontWeight.w700)),
          ),
          const SizedBox(height: 12),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return ListTile(
                  title: Text(bank['name']?.toString() ?? '', style: AppTextStyles.bodyMd.copyWith(color: AppColors.white)),
                  onTap: () {
...
class _StripeSetupState extends ConsumerState<_StripeSetup> {
  ...
  Future<void> _startStripeOnboarding() async {
    ...
    try {
      final response = await ApiService.instance.post(ApiConstants.stripeConnectOnboard, data: {});
      final data = response.data;
      final url = data is Map
          ? (data['url']?.toString() ?? data['onboarding_url']?.toString() ?? data['link']?.toString())
          : null;
      ...
      if (mounted) {
        AppSnackBar.show(context, message: 'Complete Stripe setup in your browser, then return here.', type: SnackBarType.info);
        setState(() => _isLoading = false);
      }
    } on DioException catch (e) {
      if (mounted) {
        final msg = e.response?.data is Map
            ? (e.response!.data['message'] ?? 'Could not start Stripe onboarding')
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _infoChip('Payout account (${widget.currency})', Icons.credit_card_outlined, const Color(0xFF635BFF)),
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Fast, secure payouts worldwide', style: AppTextStyles.labelMd.copyWith(color: AppColors.white, fontWeight: FontWeight.w700)),
              const SizedBox(height: 12),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Text(text, style: AppTextStyles.bodyMd.copyWith(color: AppColors.gray300)),
        ],
      ),
    );
  }
