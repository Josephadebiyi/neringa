...
export default function Support() {
  ...
  const handleSendReply = async () => {
    ...
    setSending(true);
    try {
      const res = await fetch(`${API_BASE}/${selectedTicket._id}/message`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          content: reply,
          sender: 'ADMIN',
          senderId: 'admin' // In real app, get from auth context
        })
      });
      const data = await res.json();
      if (data.success) {
        ...
        const ticketRes = await fetch(`${API_BASE}/${selectedTicket._id}`, { credentials: 'include' });
