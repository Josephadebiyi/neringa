...
// Helper to get base api without /Adminbaggo
const API_BASE = API_BASE_URL.replace('/Adminbaggo', '') + "/prices";
...
export default function PricePerKgPage() {
  ...
  const fetchPrices = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/get`, { credentials: 'include' });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingPrice
        ? `${API_BASE}/update/${editingPrice._id}`
        : `${API_BASE}/create`;
                    ...
                    <td className="py-5 px-8">
                      <div className="flex flex-col">
                        <span className="font-black text-[#1e2749] text-sm">
                          {price.currency} {price.pricePerKg.toLocaleString()}
                        </span>
