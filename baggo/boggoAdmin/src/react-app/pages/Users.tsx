...
export default function Users() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        `https://neringa.onrender.com/api/Adminbaggo/GetAllUsers?page=${currentPage}&limit=${limit}`,
        {
          method: 'GET',
          headers: {
  ...
  const handleBanToggle = async (userId: string, currentBanned: boolean) => {
    ...
    try {
      const response = await fetch(`https://neringa.onrender.com/api/Adminbaggo/banUser/${userId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
