...
export default function Users() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/GetAllUsers?page=${currentPage}&limit=${limit}&banned=${activeTab === 'banned'}`,
        {
          method: 'GET',
          headers: {
  ...
  const handleBanToggle = async (userId: string, currentBanned: boolean) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${API_BASE_URL}/banUser/${userId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleUpdateUser = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${API_BASE_URL}/updateUser/${editingUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
