...
export const sendPromoEmail = async (req, res, next) => {
  ...
  try {
    ...
    // 1. Handle Body Images (Embed as Base64 for "No Attachment" look)
    let bodyImagesHtml = '';
    if (images && Array.isArray(images)) {
      images.slice(0, 2).forEach(img => {
        bodyImagesHtml += `
          <div style="margin-top: 25px; text-align: center;">
            <img src="${img}" style="max-width: 100%; border-radius: 16px; display: block; margin: 0 auto;" />
          </div>`;
      });
    }

    // 2. Handle File Attachments (These show in the attachment tray)
      ...
      <html>
        <head>
          <style>
            ...
            .btn { display: inline-block; padding: 16px 32px; background-color: #5240E8; color: white !important; text-decoration: none; border-radius: 14px; font-weight: bold; margin-top: 30px; }
          </style>
        </head>
        <body>
          <div class="container">
            ...
            <div class="content">
              <div class="promo-text">${body}</div>
              ${bodyImagesHtml}
              <div style="text-align: center;">
                <a href="https://sendwithbago.com" class="btn">Explore Bago</a>
              </div>
            </div>
            <div class="footer">
              &copy; ${new Date().getFullYear()} Bago Logistics. All rights reserved. <br/>
              You received this email because you are a registered user of Bago.
            </div>
          </div>
        </body>
      </html>
