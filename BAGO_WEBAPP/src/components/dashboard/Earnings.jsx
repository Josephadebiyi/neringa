                    ...
                    <div className="relative z-10 flex flex-col justify-between h-full min-h-[160px]">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <h1 className="text-3xl md:text-4xl font-black tracking-tighter leading-none">{currencySymbol}{balance.toLocaleString()}</h1>
                            </div>
                    ...
                    <div>
                        ...
                        <form onSubmit={handleWithdraw} className="space-y-4">
                            <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 font-black text-[#5845D8] text-sm opacity-60">{currencySymbol}</span>
                                <input
                                    type="number"
                        ...
                        {history.slice().reverse().map((tx, i) => (
                            <div key={i} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50/50 transition-colors group">
                                ...
                                <div className={`text-right font-black text-xs tracking-tight ${tx.type === 'withdraw' ? 'text-red-500' : 'text-green-600'}`}>
                                    {tx.type === 'withdraw' ? '-' : '+'}{currencySymbol}{tx.amount.toLocaleString()}
                                </div>
                            </div>
                        ))}
                    </div>
