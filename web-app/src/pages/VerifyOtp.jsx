        ...
        <div className="min-h-screen bg-white flex overflow-hidden lg:flex-row flex-col">
            <div className="lg:w-1/2 w-full lg:min-h-screen h-[40vh] relative bg-[#012126] flex flex-col justify-between p-8 md:p-16 overflow-hidden sticky top-0">
                ...
                <div className="z-10 text-white mt-auto mb-10 md:mb-20">
                    <h1 className="text-4xl lg:text-6xl font-black mb-6 leading-tight tracking-tighter uppercase">{t('enterOtpTitle').split(' ').map((word, i) => <React.Fragment key={i}>{word} <br /></React.Fragment>)}</h1>
                    <p className="text-sm md:text-base text-white/70 max-w-md font-semibold leading-relaxed">
                        {t('sentCodeToEmailDesc')} <span className="font-bold text-white">{email}</span>.
                    </p>
                </div>
            </div>
            ...
            <div className="lg:w-1/2 w-full flex items-center justify-center p-8 bg-white z-10 lg:min-h-screen overflow-y-auto">
                <div className="w-full max-w-md py-12">
                    <h2 className="text-2xl font-black text-[#012126] mb-1.5 tracking-tight">{t('verifyOtpHeader')}</h2>
                    <p className="text-[#6B7280] font-semibold text-base mb-10">{t('verifyIdentityDesc')}</p>
                    ...
                    <form onSubmit={handleVerifySubmit} className="space-y-6">
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-black text-[#012126] uppercase tracking-widest ml-1">{t('sixDigitCodeLabel')}</label>
                            <input
                                type="text"
                                pattern="[0-9]*"
                                maxLength="6"
                                ...
                                className="w-full px-5 py-3.5 bg-[#f8f9fa] tracking-[0.8em] text-center rounded-xl border-2 border-transparent focus:border-[#5845D8] focus:bg-white outline-none transition-all text-[#012126] text-2xl font-black"
                                placeholder="000000"
                                required
                            />
                        </div>
                    ...
                    <p className="mt-12 text-center text-[#6B7280] font-bold text-sm">
                        ...
                        <button
                            ...
                            disabled={resending}
                            className="text-[#5845D8] font-black border-b-2 border-transparent hover:border-[#5845D8] transition-all ml-1 disabled:opacity-50"
                        >
