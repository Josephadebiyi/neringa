...
app.post('/api/admin/login', zValidator('json', loginSchema), async (c) => {
  try {
    ...
    if (!adminUsername || !adminPassword) {
      return c.json({ error: 'Admin auth is not configured' }, 500);
    }

    const loginKey = `${c.req.header('cf-connecting-ip') || c.req.header('x-forwarded-for') || 'unknown'}:${username}`;
    const attemptWindowMs = 15 * 60 * 1000;
    const maxAttempts = 5;
    const now = Date.now();
    const attempts = loginAttempts.get(loginKey) || [];
    const recentAttempts = attempts.filter((ts: number) => now - ts < attemptWindowMs);
