...
export default function LocationsPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let data;
            if (editingLocation) {
                data = await updateLocation(editingLocation._id, formData);
            } else {
                data = await createLocation(formData);
            }
    ...
    const handleToggleStatus = async (location: Location) => {
        try {
            const data = await updateLocation(location._id, { isActive: !location.isActive });
            if (data.success) {
