...
import { File, Image, User, Eye, Calendar, CheckCircle, XCircle, Clock } from "lucide-react";
...
export default function KYCVerificationManager() {
  ...
  const fetchKYCData = async () => {
    try {
      ...
      if (result.success && result.data && Array.isArray(result.data.finduser)) {
        // Combine user and kyc data
        const combined = result.data.finduser.map((user: UserData) => {
          const userKyc = Array.isArray(result.data.kyc)
            ? result.data.kyc.find((k: KYCData) => k.userid === user._id)
            : undefined;
          return {
            user,
            kyc: userKyc
