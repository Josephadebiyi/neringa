...
import { Package, CheckCircle, Clock, ChevronRight, XCircle, AlertCircle, Info, Truck } from 'lucide-react-native';
...
export default function NotificationsScreen() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const transformedNotifications = response.data.data.notifications.map((notif: any) => {
        ...
        let type = 'update';
        let icon = Info;
        let iconColor = '#6B6B6B';
        let title = 'Update';

        // Helper to extract title if message is "Title: Body"
        ...
        if (lowerMsg.includes('accepted')) {
          ...
          icon = CheckCircle;
          iconColor = '#4CAF50';
          if (title === 'Update') title = 'Request Accepted';
        } else if (lowerMsg.includes('rejected') || lowerMsg.includes('declined')) {
          type = 'error';
          icon = XCircle;
          iconColor = '#F44336';
          if (title === 'Update') title = 'Request Declined';
        } else if (lowerMsg.includes('approved')) {
          type = 'success';
          icon = CheckCircle;
          iconColor = '#4CAF50';
          if (title === 'Update') title = 'Approved';
        } else if (lowerMsg.includes('transit') || lowerMsg.includes('delivering') || lowerMsg.includes('shipping')) {
          type = 'shipping';
          icon = Truck;
          iconColor = '#5845D8';
          if (title === 'Update') title = 'Shipping Update';
        } else if (lowerMsg.includes('completed') || lowerMsg.includes('delivered')) {
          type = 'completed';
          icon = CheckCircle;
          iconColor = '#10B981';
          if (title === 'Update') title = 'Completed';
        } else if (lowerMsg.includes('verification') || lowerMsg.includes('kyc')) {
          type = 'security';
          icon = AlertCircle;
          iconColor = '#F59E0B';
          if (title === 'Update') title = 'Security Update';
        } else if (lowerMsg.includes('wallet') || lowerMsg.includes('payment') || lowerMsg.includes('earnings')) {
          type = 'payment';
          icon = Package;
          iconColor = '#2563EB';
          if (title === 'Update') title = 'Payment Update';
        }
