...
import { MapPin, Weight, ChevronLeft, Calendar } from 'lucide-react-native';
...
export default function SendPackageScreen() {
  ...
  // Recalculate estimated price whenever relevant inputs change
  useEffect(() => {
    const calculateEstimate = async () => {
      if (step !== 2 || !fromCountry || !toCountry || !packageWeight) {
      ...
      >
        ...
        {step === 1 ? (
          ...
          <View>
            ...
            <View style={styles.section}>
              <Text style={styles.label}>When is it ready?</Text>
              <TouchableOpacity
                style={styles.weightInput}
                onPress={() => setDatePickerVisibility(true)}
              >
                <Calendar size={20} color={'#6B6B6B'} />
                <Text style={[styles.input, !packageReadyDate && { color: '#9E9E9E' }]}>
                  {packageReadyDate || "Select date"}
                </Text>
              </TouchableOpacity>
            </View>
      ...
      <View style={styles.footer}>
        {step === 2 && (
          <View style={{ marginBottom: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <Text style={{ fontSize: 16, fontWeight: '600', color: '#1A1A1A' }}>Estimated Cost:</Text>
            {isCalculatingEstimate ? (
              <ActivityIndicator size="small" color="#5845D8" />
            ) : estimatedPrice ? (
              <Text style={{ fontSize: 20, fontWeight: 'bold', color: '#5845D8' }}>€{estimatedPrice}</Text>
            ) : (
              <Text style={{ fontSize: 16, color: '#6B6B6B' }}>Pending</Text>
            )}
          </View>
        )}
        <TouchableOpacity
