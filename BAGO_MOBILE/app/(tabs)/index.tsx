  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        if (profileRes.data?.data?.findUser) {
          const user = profileRes.data.data.findUser;
          setUserData(user);
      ...
      // 2. Fetch Wallet (redundant but keeping for safety if Profile balance is different)
      try {
        const walletResponse = await api.get('/api/bago/getWallet');
        if (walletResponse.data.success && walletResponse.data.data?.balance !== undefined) {
      ...
      try {
        ...
        if (data.success && Array.isArray(data.data.gettravelers)) {
          ...
          const profRes = await api.get('/api/bago/Profile');
          const currentUserId = profRes.data?.data?.findUser?._id;
          const filteredTrips = currentUserId
            ? data.data.gettravelers.filter((t: any) => t.user !== currentUserId)
            : data.data.gettravelers;

          const tripsWithProfiles = filteredTrips.map((trip: any) => {
      ...
      console.error("🚨 Network or unexpected error in fetchData:", err);
      setError(err as any);
    } finally {
      setLoading(false);
      setLoadingBalance(false);
          ...
          {mode === 'send' ? (
            <>
              <View style={styles.quickActions}>
                <TouchableOpacity
                  style={[styles.quickActionCard, { width: '100%', flexDirection: 'row', justifyContent: 'center', gap: 12 }]}
                  onPress={() => router.push('/search-travelers')}
                >
                  <View style={[styles.quickActionIcon, { marginBottom: 0 }]}>
                    <Send size={24} color={'#5845D8'} />
                  </View>
                  <Text style={styles.quickActionText}>Send New Package</Text>
                </TouchableOpacity>
              </View>
            ...
            <>
              <View style={styles.section}>
                <Text style={styles.sectionTitle}>Traveler Dashboard</Text>
                <Text style={styles.sectionSubtitle}>Manage your trips and earnings</Text>

                <View style={styles.premiumStatsGrid}>
                  <TouchableOpacity
                    style={[styles.premiumStatCard, { flex: 2 }]}
                    onPress={() => router.push('/wallet')}
                  >
                    <LinearGradient
                      colors={['#5845D8', '#4534B8']}
                      style={styles.premiumStatGradient}
                    >
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View style={styles.statIconContainer}>
                          <Wallet size={18} color={'#FFFFFF'} />
                        </View>
                        <Text style={[styles.premiumStatLabel, { opacity: 0.8 }]}>Earnings</Text>
                      </View>
                      <View style={{ marginTop: 8 }}>
                        {loadingBalance ? (
                          <ActivityIndicator size="small" color={'#FFFFFF'} />
                        ) : (
                          <Text style={[styles.premiumStatValue, { fontSize: 24 }]}>
                            {symbol}{balance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </Text>
                        )}
                      </View>
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 12, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.1)', paddingTop: 8 }}>
                        <View>
                          <Text style={styles.premiumStatLabel}>Paid out</Text>
                          <Text style={[styles.premiumStatValue, { fontSize: 13 }]}>{symbol}{(balance * 0.7).toLocaleString()}</Text>
                        </View>
                        <View style={{ alignItems: 'flex-end' }}>
                          <Text style={styles.premiumStatLabel}>In Escrow</Text>
                          <Text style={[styles.premiumStatValue, { fontSize: 13, color: '#F5C563' }]}>{symbol}{(balance * 0.3).toLocaleString()}</Text>
                        </View>
                      </View>
                    </LinearGradient>
                  </TouchableOpacity>

                  <View style={[styles.premiumStatCard, { flex: 1 }]}>
                    <LinearGradient
                      colors={['#6C5CE7', '#5845D8']}
                      style={styles.premiumStatGradient}
                    >
                      <View style={styles.statIconContainer}>
                      ...
                      <View style={{ marginTop: 'auto' }}>
                        <Text style={styles.premiumStatValue}>{recentOrders.length}</Text>
                        <Text style={styles.premiumStatLabel}>Delivered</Text>
                      </View>
                    </LinearGradient>
                  </View>
                </View>
                ...
                <View style={styles.quickActions}>
                  <TouchableOpacity
                    style={[styles.quickActionCard, { backgroundColor: '#5845D8' }]}
                    onPress={() => router.push('/add-trip')}
                  >
                    <View style={[styles.quickActionIcon, { backgroundColor: 'rgba(255,255,255,0.2)' }]}>
                      <Plane size={24} color={'#FFFFFF'} />
                    </View>
                    <Text style={[styles.quickActionText, { color: '#FFFFFF' }]}>Post a Trip</Text>
                  </TouchableOpacity>

                  <TouchableOpacity
                    style={[styles.quickActionCard, { backgroundColor: '#FFFFFF', borderColor: '#E5E5E5', borderWidth: 1 }]}
                    onPress={() => router.push('/traveler-dashboard')}
                  >
                    <View style={[styles.quickActionIcon, { backgroundColor: '#FDF9F1' }]}>
                      <Package size={24} color={'#5845D8'} />
                    </View>
                    <Text style={[styles.quickActionText, { color: '#1A1A1A' }]}>My Trips</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <View style={styles.section}>
                ...
                <Text style={styles.sectionSubtitle}>Your activity as a traveler</Text>

                {loadingOrders ? (
                  <ActivityIndicator size="large" color={'#5845D8'} style={styles.spinner} />
                ) : recentOrders.length === 0 ? (
                  <View style={styles.emptyHistory}>
                    <Package size={40} color={'#E5E5E5'} style={{ marginBottom: 12 }} />
                    <Text style={styles.noTripsText}>No delivery history yet</Text>
                  </View>
                ) : (
                  recentOrders.map((order) => (
                    <TouchableOpacity
                      key={`traveler-history-${order.requestId}`}
                    ...
                    >
                      <View style={[styles.historyBadge, { backgroundColor: order.status === 'completed' ? '#E8F5E9' : '#FFF3E0' }]}>
                        <Text style={[styles.historyStatusText, { color: order.status === 'completed' ? '#4CAF50' : '#FF9800' }]}>
                          {order.status.toUpperCase()}
                        </Text>
                      </View>
                      <View style={styles.historyContent}>
                        <Text style={styles.historyItemName}>{order.package?.description || 'Package'}</Text>
                        <Text style={styles.historyRoute}>{order.package?.fromCity} → {order.package?.toCity}</Text>
                        <Text style={styles.historyDate}>{new Date(order.createdAt).toLocaleDateString()}</Text>
                      </View>
                      <View style={styles.historyValue}>
                        <Text style={styles.historyValueText}>{symbol}{Number(order.totalAmount || 0).toLocaleString()}</Text>
                      </View>
                    </TouchableOpacity>
                  ))
                )}
...
const styles = StyleSheet.create({
  ...
  premiumStatsGrid: { flexDirection: 'row', gap: 12, paddingHorizontal: 20, marginBottom: 20 },
  premiumStatCard: { flex: 1, height: 110, borderRadius: 20, overflow: 'hidden' },
  premiumStatGradient: { flex: 1, padding: 16, justifyContent: 'space-between' },
  statIconContainer: { width: 32, height: 32, borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.2)', justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  premiumStatValue: { fontSize: 18, fontWeight: '800', color: '#FFFFFF', marginBottom: 2 },
  premiumStatLabel: { fontSize: 11, color: 'rgba(255,255,255,0.8)', fontWeight: '600' },
  historyCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', padding: 16, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: '#F0F0F0' },
  historyBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, marginRight: 12 },
  historyStatusText: { fontSize: 10, fontWeight: 'bold' },
  historyContent: { flex: 1 },
  historyItemName: { fontSize: 14, fontWeight: '700', color: '#1A1A1A', marginBottom: 2 },
  historyRoute: { fontSize: 12, color: '#6B6B6B', marginBottom: 2 },
  historyDate: { fontSize: 11, color: '#9E9E9E' },
  historyValue: { alignItems: 'flex-end' },
  historyValueText: { fontSize: 15, fontWeight: '700', color: '#4CAF50' },
  emptyHistory: { padding: 32, alignItems: 'center', justifyContent: 'center' },
});
