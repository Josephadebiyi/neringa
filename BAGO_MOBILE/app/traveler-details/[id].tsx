...
export default function TravelerDetailsScreen() {
  ...
  const initialTraveler = useMemo(() => ({
    id: travelerId as string,
    tripId: tripId as string,
    name: (params.name as string) || 'Traveler',
    travelerEmail: (params.travellerEmail as string) || '',
    rating: params.rating ? parseFloat(params.rating as string) : 0,
    trips: params.trips ? parseInt(params.trips as string, 10) : 0,
    verified: params.verified === 'true',
    from: (params.from as string) || 'Unknown',
    to: (params.to as string) || 'Unknown',
    date: (params.date as string) || '2025-10-15',
  ...
  const handleBookRequest = () => {
    router.push({
      ...
      params: {
        ...
        availableKg: travelerState.availableKg,
          ...
          <View style={[styles.profileCard, { backgroundColor: '#FFFFFF', borderBottomColor: '#E5E5E5' }]}>
            <View style={[styles.avatar, { backgroundColor: '#5845D8' }]}>
              <Text style={[styles.avatarText, { color: '#FFFFFF' }]}>
                {typeof traveler.name === 'string' ? traveler.name.charAt(0) : 'T'}
              </Text>
            </View>
            ...
            <View style={styles.infoGrid}>
              <View style={[styles.infoCard, { backgroundColor: '#FFFFFF' }]}>
                ...
                <Text style={[styles.infoValue, { color: '#1A1A1A' }]}>
                  {new Date(traveler.date as string).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </Text>
              </View>
              <View style={[styles.infoCard, { backgroundColor: '#FFFFFF' }]}>
