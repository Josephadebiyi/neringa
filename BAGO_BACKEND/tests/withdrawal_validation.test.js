...
function validateWithdrawal({ amount, walletCurrency, availableBalance, paypalEmail, payoutStatus, paystackRecipientCode, spentTodayUsd, amountUsd }) {
  ...
  if (method === 'paypal' && (!paypalEmail || payoutStatus !== 'active'))
    return { ok: false, reason: 'Add an active PayPal payout email before withdrawing.' };
...
describe('withdrawal validation', () => {
  ...
  it('rejects USD withdrawal when PayPal is not connected', () => {
    const r = validateWithdrawal({ ...basePaypalUser, paypalEmail: null, payoutStatus: null, amount: 20 });
    expect(r.ok).toBe(false);
    expect(r.reason).toMatch(/paypal/i);
  });

  it('rejects USD withdrawal when PayPal payout is not active', () => {
    const r = validateWithdrawal({ ...basePaypalUser, payoutStatus: 'pending', amount: 20 });
    expect(r.ok).toBe(false);
  });
