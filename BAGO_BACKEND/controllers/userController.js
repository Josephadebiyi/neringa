...
import { sendWelcomeEmail, sendAccountBannedEmail, sendAccountUnblockedEmail, generateOtpEmailHtml } from '../services/emailNotifications.js';
...
export const requestEmailChange = async (req, res) => {
  try {
    ...
    const html = generateOtpEmailHtml({
      ...
      otp,
      subtitle: `You requested to change your Bago account email to <strong style="color:#111827;">${newEmail}</strong>. Use the code below to confirm this change.`,
    ...
    if (resend) {
      await resend.emails.send({
        ...
        to: newEmail,
        subject: "Confirm your new Bago email",
        html,
      });
    }
...
export const signUp = async (req, res) => {
  try {
    ...
    const html = generateOtpEmailHtml({
      firstName,
      otp: activationOtp,
      subtitle: 'Thank you for joining Bago! Use the verification code below to complete your registration.',
    });
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    const html = generateOtpEmailHtml({
      firstName: user.firstName || 'there',
      otp,
      subtitle: `We received a request to reset the password for <strong style="color:#111827;">${email}</strong>. Use the code below to verify your identity.`,
      expiryNote: 'This code expires in 5 minutes.',
    });
...
export const resendOtp = async (req, res) => {
  try {
    ...
    const html = generateOtpEmailHtml({
      firstName: user.firstName || 'there',
      otp,
      subtitle: `We received a request to resend your verification code for <strong style="color:#111827;">${normalizedEmail}</strong>.`,
      expiryNote: 'This code expires in 5 minutes.',
    });
...
export const savePushToken = async (req, res) => {
  try {
    ...
    if (!user.pushTokens.includes(token)) {
      ...
      // We could also store platform in a separate field if needed, 
      // but the current regex-based detection in the service is sufficient for now.
      await user.save();
    }
