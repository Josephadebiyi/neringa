...
async function buildCheckoutQuote({ profile, shipmentId, packageId, tripId, currency, insurance, insuranceCost, shippingAmount }) {
  if (shipmentId) {
  ...
  // Prefer client-provided shippingAmount (includes surcharge + FX conversion)
  // Fall back to weight × pricePerKg if not provided
  const baseAmount = Number(shippingAmount) > 0
    ? toAmount(Number(shippingAmount))
    : toAmount(Number(packageDoc.packageWeight || 0) * Number(tripDoc.pricePerKg || 0));
