...
export const getIncomingRequests = async (req, res) => {
  try {
    ...
    const travelerTrips = await Trip.find({ user: travelerId }).select('_id');
    const tripIds = travelerTrips.map((trip) => trip._id);
    ...
    const incomingRequests = await Request.find({ trip: { $in: tripIds } })
      .populate('package')
      .populate('sender', 'firstName lastName email')
      .populate('traveler', 'firstName lastName email')
      .sort({ createdAt: -1 });
    ...
    const data = await Promise.all(
      incomingRequests.map(async (request) => {
        ...
        return {
          ...
          role: request.sender?._id?.toString() === travelerId.toString() ? 'sender' : 'traveler',
