...
function normalizeReviewRow(row) {
  ...
  const reviewer = row.user_id
    ? maskPublicUserName({
        ...
        tradingName: row.user_trading_name,
...
const baseTripSelect = `
  select
    ...
    p.trading_name as user_trading_name,
    ...
    case
      when coalesce(trip_stats.completed_booking_count, 0) > 0 and coalesce(trip_stats.active_shipment_count, 0) = 0 then 'paid'
